...
class ActionHelloWorld(Action):
    ...
    def name(self) -> Text:
        return "action_return_symptoms"

    def __init__(self) -> None:
        # Defining dictionary for mapping to category ids
        self.code_dict = {'common':f'Symptoms(common)', 'male':f'Symptoms(male)', 'female':f'Symptoms(female)'
        , 'old people':f'Symptoms(old people)', 'critical_stage':f'Symptoms(critical_stage)', 'babies/infants':f'Symptoms(babies/infants)'}
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        condition_val = tracker.get_slot("condition")

        
        df = pd.read_csv(r"D:\19. Omdena\0. Chatbot for admitted patients\chatbot_demo\merged_data.csv")
        df.fillna('', inplace = True)
        symptom_type = self.code_dict[tracker.get_slot('symptom')]
        col_name = f'{symptom_type}'
        for i, j in df.iterrows():
            if j['condition'] == f'{condition_val}':
                dispatcher.utter_message(text=(str(j[col_name])))

        # output = df.query(f"condition == '{condition_val}'")[{symptom_type}].dropna().values.tolist()
        # for i in output:
...
#         buttons = [
#                 {"payload": "/faq/condition_symptoms{'condition': 'dengue'}", },
#                 {"payload": "/faq/condition_symptoms{'condition': 'dengue'}"},
